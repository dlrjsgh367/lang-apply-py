...
class NormalContractDialogWidget extends ConsumerStatefulWidget {
  const NormalContractDialogWidget({
    super.key,
    required this.uuid,
  ...
  @override
  ConsumerState<NormalContractDialogWidget> createState() =>
      _NormalContractDialogWidgetState();
}

class _NormalContractDialogWidgetState
    extends ConsumerState<NormalContractDialogWidget> with Files {
  ...
  createPdf() async {
    ...
    final pdfFirstColumn = await ContractTemplate.returnNormalFistContract(
        params, user, signImgData!, null, false, 'STANDARD', null);


    String today = DateFormat('yyyyMMdd').format(DateTime.now());

    String documentTitle =
        '${localization.standardLaborContract}_${params['ccdEmployeeName']}_${user!.companyInfo!.name}_$today';
  ...
  sendContract() async {
    if (widget.chatUsers.isNotEmpty && pdfFile != null) {
      var apiUploadResult = await ref
          ...
          .createContract(widget.uuid, 'STANDARD', params, 1);
  ...
  updateContract() async {
    if (widget.chatUsers.isNotEmpty && pdfFile != null) {
      var apiUploadResult = await ref
          .read(chatControllerProvider.notifier)
          .createContract(widget.uuid, 'STANDARD', params, 2);
      setState(() {
        isRunning = false;
      });
  ...
  sendUpdateDocumentMsg(int caIdx) async {
    ...
    String chatValue = localization.urgentStandardContractModified(createTime);
      ...
      case 'paymentCycleMonth':
        if (params['ccdPaymentCycleType'] == 'MONTH') {
          if (params['ccdPaymentCycleMonth'] == null ||
            ...
            if (params['ccdPaymentCycleMonth'].runtimeType == String) {
              paymentCycleMonth = params['ccdPaymentCycleMonth'].isNotEmpty
                  ? int.parse(params['ccdPaymentCycleMonth'])
                  : 0;
            } else {
              paymentCycleMonth = params['ccdPaymentCycleMonth'];
            }
      ...
      child: Stack(
        children: [
          PopScope(
            ...
            child: Scaffold(
              appBar: ModalAppbar(
                title: widget.msgKey != null ? localization.standardLaborContractEdit : localization.standardLaborContractCreate,
              ),
              body: isLoading
                  ? const Loader()
                  : Column(
                                            ...
                                            for (var data
                                              ...
                                              GestureDetector(
                                                ...
                                                child: Container(
                                                  ...
                                                  height: 48.w,
                                                  decoration: BoxDecoration(
                                                      color: params['holidayLists'].contains(data['key']) ||
                                                              params['holidayLists']
                                                                  .contains({
                                                                'hmDayOfWeek':
                                                                    data['key']
                                                      ...
                                                      borderRadius:
                                                          BorderRadius.circular(
                                                              6.w),
                                                      border: Border.all(
                                                          width: params['holidayLists'].contains(data['key']) ||
                                                                  params['holidayLists']
