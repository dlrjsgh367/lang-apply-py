...
class DefineListWidget2 extends ConsumerStatefulWidget {
  const DefineListWidget2(
      {required this.title,
  ...
  final String title;
  final List<DefineModel> defineList;
  ...
  @override
  ConsumerState<DefineListWidget2> createState() => _DefineListWidgetState();
}

class _DefineListWidgetState extends ConsumerState<DefineListWidget2> {
  ...
  searchDefine(String input, int page) async {
    ApiResultModel result = await ref
        ...
        .searchDefine(DefineEnum.industry, input);
    ...
    setState(() {
      if (selectedKeyList.contains(item.key)) {
        ...
        if (selectedList.length < widget.maxLength) {
          if (isAll) {
            if (item.depth == 2) {
              removeParentAll(item, item.depth);
            }
  ...
  returnSearchName(DefineModel item) {
    ...
    if (item.parent != null && item.parent!.key != 0) {
      name = '${item.name} ( ${setParentName('', item.parent)} )';
    }
    return name;
  }

  setParentName(String name, DefineModel? item) {
    String jobName = name;
    if (item != null) {
      if (jobName != '') {
      ...
      if (item.parent != null && item.parent!.key != 0) {
        jobName = setParentName(jobName, item.parent);
      }
    }
    return jobName;
  }
                                        ...
                                        ListView.builder(
                                          ...
                                          itemBuilder:
                                            ...
                                            final twoDepthData = oneDepthData
                                                .child[depthTwoIndex];
                                            return twoDepthData.child.isNotEmpty
                                                ? CollapseBtn(
                                                    openColor:
                                                        CommonColors.grayF2,
                                                    paddingLeft: 32,
