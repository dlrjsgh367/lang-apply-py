...
class _EventDetailScreenState extends ConsumerState<EventDetailScreen> {
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertConfirmDialog(
            ...
            alertContent: localization.eventDoesNotExist,
  ...
  applyEvent(int eventKey, EventJoinTypeEnum eventJoinType) async {
    ...
    if (eventJoinType == EventJoinTypeEnum.comment) {
      if (!isEmptyOrSpaces(commentController.text)) {
        ...
        showDefaultToast(localization.enterCommentContent);
    ...
    if (result.status == 200) {
      if (result.type == 1) {
        showDefaultToast(localization.submissionCompleted);
        if (eventJoinType == EventJoinTypeEnum.comment) {
          commentController.text = '';
          page = 1;
      ...
      showDefaultToast(localization.duplicateSubmissionNotAllowed);
      return false;
    } else {
      showDefaultToast(localization.submissionFailed);
      return false;
    }
  }
  ...
  reportEventComment(EventCommentModel eventComment) async {
    ...
    if (result.status == 200) {
      ...
      showDefaultToast(localization.cannotReportOwnContent);
    } else if (result.status == 409) {
      showDefaultToast(localization.reportLimitOncePerDay);
    } else {
  ...
  deleteEventComment(int key) async {
    ...
    if (result.status == 200) {
      page = 1;
      getEventDetailComment(int.parse(widget.idx), page);
      showDefaultToast(localization.commentDeleted);
    }
  }
  ...
  updateEventComment(int key, String comment) async {
    ...
    if (result.status == 200) {
      page = 1;
      getEventDetailComment(int.parse(widget.idx), page);
      showDefaultToast(localization.commentUpdated);
      context.pop();
    }
  }
                              ...
                              (context, index) {
                                ...
                                return eventCommentItem.isBlocked
                                    ...
                                    : EventDetailCommentListWidget(
                                        ...
                                        reportComment: () {
                                          showReportAlert(localization.report, () {
                                            reportEventComment(
                                                eventCommentItem);
                                          });
                                        },
