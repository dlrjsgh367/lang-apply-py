...
class _MyRecruitScreenState extends ConsumerState<MyRecruitScreen>
  ...
  likeAfterScrapFunc(int key) {
    ...
    if (likeList.contains(key)) {
      likeList.remove(key);
      showDefaultToast(localization.removedFromScrapJobPosts);
    } else {
      likeList.add(key);
      showDefaultToast(localization.savedToScrapJobPosts);
    }
    setState(() {
      ref
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const CommonAppbar(
        title: localization.scrapAndRecentlyViewedPosts,
      ),
      body: Column(
          ...
          Expanded(
            child: isLoading
                ...
                : jobpostList.isEmpty
                    ? CommonEmpty(
                        text: activeTab == 0
                            ? localization.noScrapJobPosts
                            : localization.noRecentlyViewedJobPosts)
                    : Stack(
