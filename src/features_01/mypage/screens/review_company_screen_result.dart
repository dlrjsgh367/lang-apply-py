...
import 'package:chodan_flutter_app/features/evaluate/widgets/evaluation_jobseeker_bottom_sheet.dart';
...
import 'package:chodan_flutter_app/features/evaluate/widgets/evaluated_jobseeker_bottom_sheet.dart';
...
class ReviewCompanyScreen extends ConsumerStatefulWidget {
  const ReviewCompanyScreen({super.key});

  @override
  ConsumerState<ReviewCompanyScreen> createState() =>
      _ReviewCompanyScreenState();
}

class _ReviewCompanyScreenState extends ConsumerState<ReviewCompanyScreen> {
  int activeTab = 0;
  bool isLoading = true;
  ...
  // 미평가
  void showEvaluationModal(String companyName, int ratingKey) {
    showModalBottomSheet(
      ...
      builder: (BuildContext context) {
        return EvaluationJobseekerBottomSheet(
          company: companyName,
          ratingKey: ratingKey,
  ...
  // 평가완료
  void showEvaluatedModal(String companyName, int ratingKey) {
    showModalBottomSheet(
      context: context,
      backgroundColor: CommonColors.white,
      elevation: 0,
      ...
      // enableDrag: false,
      builder: (BuildContext context) {
        return EvaluatedJobSeekerBottomSheet(
          company: companyName,
          ratingKey: ratingKey,
        );
      },
  ...
  getUnratedEvaluateData(int page) async {
    ...
    if (userInfo != null) {
      ApiResultModel result = await ref
          ...
          .getCompanyEvaluateData(page, userInfo.key, 'unrated');
  ...
  getRatedEvaluateData(int page) async {
    ...
    if (userInfo != null) {
      ApiResultModel result = await ref
          .read(mypageControllerProvider.notifier)
          .getCompanyEvaluateData(page, userInfo.key, 'rated');
      if (result.status == 200) {
        if (result.type == 1) {
          List<RatingModel> resultData = result.data;
          setState(() {
                ...
                activeTab == 0
                    ? Expanded(
                        child: unratedList.isNotEmpty
                            ...
                            : const CommonEmpty(
                                // TODO 디자인
                                text: localization.noDataAvailable,
                              ))
                    : Expanded(
